...
function Layout() {
  ...
  // Determine label for booking button based on role
  const isAdmin = user?.role === "admin";
  const isTrainer = user?.role === "trainer";
  const bookingLabel = isTrainer ? "My Sessions" : "My Bookings";
  ...
  return (
    <main className="max-w-[430px] min-h-screen mx-auto shadow">
      ...
      <nav className="dock max-w-[430px] mx-auto">
        ...
        {/* Conditionally render booking button */}
        {!isAdmin && (
          <button
          ...
          >
            ...
            <span className="dock-label">{bookingLabel}</span>
          </button>
        )}
